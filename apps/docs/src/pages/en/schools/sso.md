...
> This feature is in beta; you may encounter bugs.

To use this feature on the hosted service at [aurogurukul.app](https://aurogurukul.app), you need to be on the Enterprise plan. For self-hosted instances, this feature is available by default.

## Steps to set up SSO

1. Subscribe to the [Enterprise](https://app.aurogurukul.app/account/billing) plan, if you haven't, to unlock the feature. Ignore this step for self-hosted instances.

2. In the Aurogurukul dashboard, go to `Settings` -> `Miscellaneous` -> `Login providers`.
...
4. In the `SSO Provider` screen, use the `School Settings` to configure your IdP provider. Refer to the sections below to see how to configure your IdP provider.
    ...
    - **SAML ACS URL**: This is the URL that your IdP will send the SAML response to. This is usually `https://<school>.aurogurukul.app/api/auth/sso/saml2/sp/acs/sso`
    - **Audience URI (SP Entity ID)**: This is the URL that your IdP will use to validate the SAML response. This is usually `https://<school>.aurogurukul.app/api/auth/sso/saml2/sp/metadata?providerId=sso`
...
5. After configuring the IdP provider, obtain the required settings from it and populate the values in the `IDP Configuration` panel.
    ...
    - **Entry point**: This is the URL Aurogurukul will use to send the SAML request to your IdP.
...
5. In the `Configure SAML` tab, enter the `SAML ACS URL` (obtained from Aurogurukul) in the `Single sign-on URL` field and `Audience URI (SP Entity ID)` (obtained from Aurogurukul) in the `Audience URI (SP Entity ID)` field and click on `Next`.
...
5. In the SAML setup screen, click the pencil icon in the `Basic SAML Configuration` card and enter Aurogurukul's `Audience URI (SP Entity ID)` in `Identifier (Entity ID)` and the `SAML ACS URL` in `Reply URL (Assertion Consumer Service URL)`.
6. Next, obtain the IdP details from the `SAML Certificates` pane (on the same page). Click the pencil icon to open the settings pane, download the PEM certificate and the Federated certificate XML, and paste them into Aurogurukul's `Certificate` and `IDP Metadata` fields respectively.
   ![Microsoft Entra ID's certificates](/assets/schools/idp/entra-id/sso-entra-idp-cert-and-metadata.png)
7. Obtain the `Login URL` from the same screen and enter it into Aurogurukul's `Entry point` field.
...
#### a. Cloud-hosted (aurogurukul.app)

You can re-enable the email provider from the [Aurogurukul](https://app.aurogurukul.app) dashboard.
