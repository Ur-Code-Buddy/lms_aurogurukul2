...
description: Learn how to get paid via Aurogurukul
layout: ../../../layouts/MainLayout.astro
---

Receiving payments is effortless in Aurogurukul, and you get to keep 100% of what you make (except for the charges from your payment platform).

Aurogurukul offers integrations with the following payment platforms:
...
3. In your Aurogurukul school, go to the `Settings > Payment` tab and select `Stripe` in the `Payment Method` dropdown.
...
8. In the webhook dialog, perform the following actions:
    ...
    - In the destination, enter your Aurogurukul school's webhook endpoint (listed in the same payment screen in your school).
      ![Stripe webhook destination](/assets/schools/stripe-webhook-entry.png)
...
4. In your Aurogurukul school, go to the `Settings > Payment` tab and select `Razorpay` in the `Payment Method` dropdown.
...
9. In the webhook dialog, enter the following:
    - The webhook URL for your Aurogurukul school (listed in the same payment screen in your school).
...
> Lemon Squeezy does not support creating custom products on the fly. This integration works within those constraints and is treated as experimental. If checkout or webhooks fail, verify variant IDs, store settings, and webhook delivery in the Lemon Squeezy dashboard.
...
3. In the sidebar form that slides in, enter the product name and create three variants as described below.
    ...
    - **A variant with one-time pricing**: To enable one-time payments in Aurogurukul
    - **A variant with subscription pricing with a monthly frequency**: To enable monthly subscriptions and EMIs in Aurogurukul
    - **A variant with subscription pricing with a yearly frequency**: To enable yearly subscriptions in Aurogurukul  
...
5. In your Aurogurukul school's dashboard, go to `Settings > Payment` and configure the settings as described below.  
   ![Aurogurukul Lemon Squeezy config](/assets/schools/lemonsqueezy-config.png)
    ...
    3. **Lemon Squeezy Store ID**: In the Lemon Squeezy dashboard, go to `Settings > Stores` as shown below. Copy and paste this ID into the Aurogurukul settings.  
       ![Lemon Squeezy store ID](/assets/schools/lemon-store-id.png)
    4. **One-time variant ID**: In the Lemon Squeezy dashboard, go to `Products` and click on the product you configured in the steps above. In the slider popup, scroll down to the `Variants` section, click the three-dot menu for the one-time variant, and then click `Copy ID`. Paste this ID into the Aurogurukul settings.
    5. **Subscription (Monthly) variant ID**: Do the same as #4.
    6. **Subscription (Yearly) variant ID**: Do the same as #4.
    7. **Lemon Squeezy Key**: In the Lemon Squeezy dashboard, go to `Settings > API` and click on the `+` icon to generate a new key. Paste this key into the Aurogurukul settings.  
...
For how payments are processed in code, see the web application under `apps/web` (API routes and payment integrations).
