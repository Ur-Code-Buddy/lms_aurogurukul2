...
1. In the Aurogurukul dashboard, go to `Settings` -> `Miscellaneous` -> `Login providers`.
...
8. In the OAuth client configuration screen, use the values from Aurogurukul:

    - Add the `Authorized JavaScript origin` shown in Aurogurukul.
    - Add the `Authorized redirect URI` shown in Aurogurukul.

9. Click `Create`, then copy the generated `Client ID` and `Client secret`.

10. Return to Aurogurukul and paste those values into the `Google App Configuration` card.

11. Click `Save`.

12. Go back to the `Login providers` screen and enable the Google provider.

![Aurogurukul google login provider](/assets/schools/google-login-checkbox.png)
...
Disabling the Google provider does not delete the users who previously signed up with Google. Their Aurogurukul account, purchases, and progress remain intact.
...
Make sure the `Authorized redirect URI` in Google Cloud Console exactly matches the value shown in Aurogurukul, including the protocol (`https://`) and the full path.

### 2. I get an origin mismatch error

Make sure the `Authorized JavaScript origin` in Google Cloud Console exactly matches the value shown in Aurogurukul.
