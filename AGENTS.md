...
- `apps/web` (`@aurogurukul/web`): primary product app (Next.js app router), GraphQL API (`apps/web/graphql`), REST endpoints (`apps/web/app/api`), admin/public UI, auth, and payment entrypoints.
- `apps/queue` (`@aurogurukul/queue`): async/background job processing.
- `apps/docs` (`@aurogurukul/docs`): product documentation site.
- `packages/common-models` (`@aurogurukul/common-models`): shared domain types/constants used across apps/packages.
- `packages/orm-models` (`@aurogurukul/orm-models`): shared DB schemas/models for Mongo-backed entities.
- `packages/common-logic` (`@aurogurukul/common-logic`): shared business logic that should not stay app-specific.
- `packages/components-library` (`@aurogurukul/components-library`): reusable UI components/hooks (e.g. forms, media selector, upload hooks).
- `packages/page-blocks` (`@aurogurukul/page-blocks`): page builder blocks/widgets and related rendering logic.
- `packages/page-primitives` (`@aurogurukul/page-primitives`): foundational themed UI primitives.
- `packages/page-models` (`@aurogurukul/page-models`): theme/page model definitions consumed by rendering/editor flows.
- `packages/text-editor` (`@aurogurukul/text-editor`): rich text editor used in admin/content editing.
- `packages/email-editor` (`@aurogurukul/email-editor`): email template/editor model and rendering structures.
- `packages/utils` (`@aurogurukul/utils`): shared utility helpers.
- `packages/icons` (`@aurogurukul/icons`): shared icon exports.
- `packages/scripts` (`@aurogurukul/scripts`): reusable maintenance scripts (not one-off migrations).
